...
export function TemplateClassique({ restaurant, menu }: Props) {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: "#0f0705", color: "#f6ded3" }}>
      {/* Header */}
      <header className="sticky top-0 z-30 backdrop-blur-md border-b" style={{ borderColor: "#3d2418", backgroundColor: "rgba(15,7,5,0.9)" }}>
        ...
        {/* Category tabs */}
        <div className="border-t overflow-x-auto scrollbar-none" style={{ borderColor: "#3d2418" }}>
          <div className="max-w-lg mx-auto flex gap-1 px-3 py-1.5 min-w-max">
            {menu.categories.map(cat => (
              <button key={cat.id} onClick={() => setActiveCat(cat.id)}
                className={cn(
                  "px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all",
                  activeCat === cat.id ? "text-white" : "opacity-50 hover:opacity-80"
                )}
                style={activeCat === cat.id ? { backgroundColor: accent } : {}}>
        ...
        {activeItems.length === 0 ? (
          ...
          activeItems.map(item => (
            <button key={item.id} onClick={() => setSelectedItem(item)}
              ...
              style={{ borderColor: "#3d2418", backgroundColor: "rgba(26,15,10,0.6)" }}>
              {item.imageUrl && (
      ...
      {/* Footer */}
      <footer className="border-t py-6 px-4" style={{ borderColor: "#3d2418" }}>
